...
async def login_handler(c: Client, m: Message):
    try:
        try:
            ...
            await ag.edit("I can't wait more for password, 6969 is the pass")
            return
        if textp == MY_PASS:
            await pass_db.add_user_pass(m.chat.id, textp)
            ag_text = "yeah! you entered the password correctly"
        else:
            ag_text = "Wrong password, type 6969 "
...
async def private_receive_handler(c: Client, m: Message):
    if MY_PASS:
        ...
        if check_pass== None:
            await m.reply_text("ʟᴏɢɪɴ ғɪʀsᴛ ᴜsɪɴɢ /login ᴄᴍᴅ(ᴛʏᴘᴇ /login 6969 ")
            return
...
async def channel_receive_handler(bot, broadcast):
    if MY_PASS:
        ...
        if check_pass == None:
            await broadcast.reply_text("Login first using /login cmd \n don\'t know the pass? here it is 6969")
            return
        if check_pass != MY_PASS:
            await broadcast.reply_text("Wrong password, login again 6969 is pass")
